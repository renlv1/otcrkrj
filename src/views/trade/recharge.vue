  ...
  <div class="c-page">
    <div class="wrap">
      ...
      <el-dialog class="d-form" :visible.sync="showDialog">
        ...
        <div class="intro">当前选择的交易单价：<span class="cblue">{{currentOrder.rate | f}}CNY</span>, 请输入充值金额或支付金额进行下单。</div>
...
export default {
  ...
  methods: {
    ...
    submit () {
      this.$fetch.post('/deposit/userDeposit', {
        sellorderid: this.currentOrder.id,
        total: this.amount,
        loading: true
      }).then(res => {
        if (res.success) {
          this.$message.success('下单成功！')
          this.$router.push(`/order/rechargeDetail?id=${res.data.id}`)
        } else {
          this.$message.error({
            message: res.msg,
          })
